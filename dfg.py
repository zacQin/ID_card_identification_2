...
input_img_file = "/Users/qinfeiyu/PycharmProjects/IDcard/test_result2/3E3BF85D54DC11CFFCC1964440A4C56B/result_lower.png"
...
def rotate_image(image,img,img1):
    degree = CalcDegree(image)
    print("调整角度：", degree)
    if degree is not 'No':
        rotate = rotateImage(img, degree)
        rotate1 = rotateImage(img1, degree)
    # cv2.imshow("rotate", rotate)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()image.png
        tag=1
        return rotate,rotate1,tag
    else:
        tag=0
        return img,img1,tag
